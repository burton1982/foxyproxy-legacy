...
function updateTemplateExample(controlName, exampleControlName, saObj) {
  controlName && (document.getElementById(controlName).value = saObj.urlTemplate);    
	exampleControlName.value = saObj.applyTemplate(foxyproxy.getMessage("foxyproxy.pattern.template.example.label2"));
}
...
function onAutoAddEnabled(cb) {
	if (cb.checked) {
		if (foxyproxy.autoadd.allowed()) {
		  ...
		  overlay.alert(this, foxyproxy.getMessage("superadd.verboten", [foxyproxy.getMessage("foxyproxy.tab.autoadd.label")]));
		  cb.checked = false;
		}
	}
	else {
	...
	if (cb.checked) {
		if (foxyproxy.quickadd.allowed()) {
	  	foxyproxy.quickadd.enabled = true;
			...
			//_updateQuickAddProxyMenu();
		  //overlay.alert(this, foxyproxy.getMessage("autoadd.notice"));
		}
		else {
		  overlay.alert(this, foxyproxy.getMessage("superadd.verboten", [foxyproxy.getMessage("foxyproxy.quickadd.label")]));
		  cb.checked = false;
		}
	}
	else {
