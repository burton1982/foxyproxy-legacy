...
ManualConf.prototype = {
  ...
  fromDOM : function(node) {
    this._host = node.hasAttribute("host")? node.getAttribute("host") :
    	node.getAttribute("http") || node.getAttribute("socks"); // "host" is new for 2.5
    this._port = node.hasAttribute("httpport")? node.getAttribute("httpport") :
    	node.getAttribute("port") || node.getAttribute("socksport"); // "port" is new for 2.5
    this._socksversion = node.getAttribute("socksversion");
	  this._isSocks = node.hasAttribute("isSocks") ? node.getAttribute("isSocks") == "true" : node.getAttribute("socks") != "";	// new for 2.5
	  this._makeProxy();
  },
