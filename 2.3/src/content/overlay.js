...
var foxyproxy = {
  ...
  onLoad : function() {
    ...
    var notDisabled = this.fp.mode != "disabled";
    notDisabled && this.toggleFilter(true);
    notDisabled && this.fp.autoadd.enabled && this.togglePageLoad(true);
  ...
  toggleToolsMenu : function(e) {
 	 	document.getElementById("foxyproxy-tools-menu-1").hidden = !e;
  },

  toggleContextMenu : function(e) {
 	 	document.getElementById("foxyproxy-context-menu-1").hidden = !e;
  },
  ...
  toggleFilter : function(enabled) {
    var ps = Components.classes["@mozilla.org/network/protocol-proxy-service;1"]
      .getService(Components.interfaces.nsIProtocolProxyService);
    if (enabled) {
      ps.unregisterFilter(this); // safety
  ...
  togglePageLoad : function(e) {
	...
	deadEnd : function(e) {
	  this.alert(null, this.fp.getMessage("error.noload", [e]));
    return this.fp.proxies.deadEnd;
	},

  applyFilter : function(ps, uri, proxy) {
    try {
      var ret = this.fp.getMatchingProxy(uri);
      ret = ret.getProxy(uri);
      !ret && (ret = this.fp.proxies.item(this.fp.proxies.length-1)); // Failsafe: use lastresort proxy if nothing else was chosen
      return ret;
    }
    catch (e) {
      dump(e + "\n");   
      return this.deadEnd(e);      
    }
  },
  ...
  toggleStatusBarIcon : function(e) {
      document.getElementById("foxyproxy-status-icon").hidden = !e;                
  },
  ...
  setMode : function(mode) {
	  var m = mode != "patterns" && mode != "disabled" && mode != "random" ? "static" : mode;
    document.getElementById("foxyproxy-tools-menu-1") && document.getElementById("foxyproxy-tools-menu-1").setAttribute("mode", m);
 	  document.getElementById("foxyproxy-context-menu-1") && document.getElementById("foxyproxy-context-menu-1").setAttribute("mode", m);
    document.getElementById("foxyproxy-button-1") && document.getElementById("foxyproxy-button-1").setAttribute("mode", m);
    document.getElementById("foxyproxy-status-icon").setAttribute("mode", m);
    var self=this;
    switch(m) {
      case "patterns":
      	_adorn(this.fp.getMessage("foxyproxy.tab.patterns.label"), "#E78500");
        break;
      case "disabled":
        _adorn(this.fp.getMessage("disabled"), "red");
        break;
      case "random":
      	_adorn(this.fp.getMessage("random"), "purple");
        break;          
      default:
      	_adorn(this.fp._selectedProxy.name, "blue");
    };
    function _adorn(m, c) {
      var e = document.getElementById("foxyproxy-status-text");
      e.setAttribute("label", self.fp.getMessage("foxyproxy") + ": " + m);
  ...
  onPopupShowing : function(menupopup, evt) {
    ...
    if (this._popupShowing == 1) {
      ...
      itm.setAttribute("style", "color: #E78500;");
      checkOne.push(itm);
      ...
      if (this.fp.advancedMenus) {
	      ...
	      _createCheckMenuItem(gssubmenupopup,
	        "foxyproxy.fp.animatedIcons=!foxyproxy.fp.animatedIcons;foxyproxy.updateViews(false);",
	        this.fp.pacErrorNotification,
  ...
  /*wpListener : {
    enable : function(e) {
      // todo: make sure to check foxyproxy.enabled == true !
      ...
      if (e) {
        browser.gBrowser.addProgressListener(this, Components.interfaces.nsIWebProgress.NOTIFY_STATE_DOCUMENT);      
      }
      else {
        browser.gBrowser.removeProgressListener(this);  
      }
    },
...
window.addEventListener("unload", function(e) { foxyproxy.toggleFilter(false);foxyproxy.togglePageLoad(false); }, false);
